...
function EnterProperty(name, owner)
	...
	TriggerServerEvent('::{razzway.xyz}::esx_property:saveLastProperty', name)
...
function ExitProperty(name)
	...
	TriggerServerEvent('::{razzway.xyz}::esx_property:deleteLastProperty')
					...
					if PropertyIsOwned(property) then
						...
						if not Config.EnablePlayerManagement then
							RageUI.ButtonWithStyle("Vendre sa propriété", nil, {RightLabel = "→"}, true, function(h,a,s)
								if s then
									TriggerServerEvent('::{razzway.xyz}::esx_property:removeOwnedProperty', property.name)
									FreezeEntityPosition(GetPlayerPed(-1), false)
									RageUI.CloseAll()
								end
							end)
						...
						RageUI.ButtonWithStyle("Acheter la propriété", nil, {RightLabel = "→"}, true, function(h,a,s)
							if s then
								TriggerServerEvent('::{razzway.xyz}::esx_property:buyProperty', property.name)
								FreezeEntityPosition(GetPlayerPed(-1), false)
								RageUI.CloseAll()
							end
						end)
					...
					for i = 1, #gatewayProperties, 1 do
						if PropertyIsOwned(gatewayProperties[i]) then
							...
							RageUI.ButtonWithStyle("Nom: "..label, nil, {RightLabel = "Vendre →→"}, true, function(h,a,s)
								if s then
									TriggerServerEvent('::{razzway.xyz}::esx_property:removeOwnedProperty', value)
									RageUI.CloseAll()
									FreezeEntityPosition(GetPlayerPed(-1), false)
								end
							end)
...
RMenu:Get('propertyavailable', 'available').Closed = function() sPropertyAvailable.Menu = false FreezeEntityPosition(GetPlayerPed(-1), false) end
					...
					for i = 1, #gatewayProperties, 1 do
						if not PropertyIsOwned(gatewayProperties[i]) then
							...
							RageUI.ButtonWithStyle("Nom: "..label, nil, {RightLabel = "Achetez →→"}, true, function(h,a,s)
								if s then
									TriggerServerEvent('::{razzway.xyz}::esx_property:buyProperty', value)
									RageUI.CloseAll()
									FreezeEntityPosition(GetPlayerPed(-1), false)
								end
							end)
					...
					for i = 1, #gatewayProperties, 1 do
						if not PropertyIsOwned(gatewayProperties[i]) then
							...
							RageUI.ButtonWithStyle("Nom: "..label, nil, {RightLabel = "Louez →→"}, true, function(h,a,s)
								if s then
									TriggerServerEvent('::{razzway.xyz}::esx_property:rentProperty', value)
									RageUI.CloseAll()
									FreezeEntityPosition(GetPlayerPed(-1), false)
								end
							end)
				...
				RageUI.IsVisible(RMenu:Get('roommenu', 'invitez'), true, true, true, function()
					...
					for i = 1, #playersInArea, 1 do
						...
						if playersInArea[i] ~= PlayerId() then
							label = GetPlayerName(playersInArea[i])
							value = playersInArea[i]
						end
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_dressing', {
					...
					TriggerEvent('::{razzway.xyz}::skinchanger:getSkin', function(skin)
						ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPlayerOutfit', function(clothes)
							...
							TriggerEvent('::{razzway.xyz}::skinchanger:getSkin', function(skin)
								TriggerServerEvent('::{razzway.xyz}::esx_skin:save', skin)
							end)
		...
		elseif data.current.value == 'remove_cloth' then
			ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPlayerDressing', function(dressing)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'remove_cloth', {
					...
					menu2.close()
					TriggerServerEvent('::{razzway.xyz}::esx_property:removeOutfit', data2.current.value)
						...
						--ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPropertyInventory', function(inventory)
							if inventory.dirtycash > 0 then
								...
								RageUI.ButtonWithStyle("Récupérez votre argent sale", nil, {RightLabel = ESX.Math.GroupDigits(inventory.dirtycash)}, true, function(h,a,s)
									if s then
										local q = inventory.dirtycash
										local quantity = tonumber(q)
										if quantity ~= nil then
											TriggerServerEvent('::{razzway.xyz}::esx_property:getItem', owner, type, value, quantity)
											RageUI.CloseAll()
										else
						...
						--ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPropertyInventory', function(inventory)
							for i = 1, #inventory.weapons, 1 do
								...
								RageUI.ButtonWithStyle("Arme: "..label, nil, {RightLabel = ammo}, true, function(h,a,s)
									if s then
										TriggerServerEvent('::{razzway.xyz}::esx_property:getItem', owner, type, value, ammo)
										RageUI.CloseAll()
									end
								end)
							end
		...
		--[[ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPropertyInventory', function(inventory)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'room_inventory', {
				...
				if data.current.type == 'item_weapon' then
					...
					TriggerServerEvent('::{razzway.xyz}::esx_property:getItem', owner, data.current.type, data.current.value, data.current.ammo)
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'get_item_count', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('::{razzway.xyz}::esx_property:getItem', owner, data.current.type, data.current.value, quantity)
							ESX.SetTimeout(300, function()
								OpenRoomInventoryMenu(property, owner)
							end)
						end
			...
			Citizen.CreateThread(function()
				while sInventoryMenuD.Menu do
					--RageUI.IsVisible(RMenu:Get('sinventoryDeposer', 'deposer'), true, true, true, function()
						--RageUI.ButtonWithStyle("Objets", nil, {RightLabel = "→"}, true, function(h,a,s)
							...
							RageUI.ButtonWithStyle("Déposez votre argent", nil, {RightLabel = "→"}, true, function(h,a,s)
								if s then
									if inventory.dirtycash > 0 then
										...
										local quantity = tonumber(q)
										if quantity ~= nil then
											TriggerServerEvent('::{razzway.xyz}::esx_property:putItem', owner, type, value, quantity)
											RageUI.CloseAll()
										else
											ESX.ShowNotification(_U('amount_invalid'))
										end
								...
								--if s then
									for i = 1, #inventory.weapons, 1 do
										...
										RageUI.ButtonWithStyle("Arme: "..label, nil, {RightLabel = "→"}, true, function(h,a,s)
											if s then
												TriggerServerEvent('::{razzway.xyz}::esx_property:putItem', owner, type, value, ammo)
												RageUI.CloseAll()
											end
										end)
									end
						...
						--ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPlayerInventory', function(inventory)
							...
							for i = 1, #inventory.items, 1 do
								...
								if item.count > 0 then
									...
									local type = 'item_standard'
									local value = item.name
									RageUI.ButtonWithStyle("Item: "..label, nil, {RightLabel = "→"}, true, function(h,a,s)
										if s then
											local q = item.count
											local quantity = tonumber(q)
											if quantity ~= nil then
		...
		--[[ESX.TriggerServerCallback('::{razzway.xyz}::esx_property:getPlayerInventory', function(inventory)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'player_inventory', {
				...
				if data.current.type == 'item_weapon' then
					...
					TriggerServerEvent('::{razzway.xyz}::esx_property:putItem', owner, data.current.type, data.current.value, data.current.ammo)
					ESX.SetTimeout(300, function()
						OpenPlayerInventoryMenu(property, owner)
					end)
				else
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'put_item_count', {
						...
						if quantity == nil then
							...
							TriggerServerEvent('::{razzway.xyz}::esx_property:putItem', owner, data.current.type, data.current.value, tonumber(data2.value))
							ESX.SetTimeout(300, function()
								OpenPlayerInventoryMenu(property, owner)
							end)
						end
