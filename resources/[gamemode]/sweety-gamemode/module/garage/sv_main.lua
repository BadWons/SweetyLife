...
MySQL.ready(function()
	MySQL.Async.execute('UPDATE owned_vehicles SET state = true WHERE state = false', {}, function(rowsChanged)
		if rowsChanged > 0 then
			print(('Garage: %s vehicules ont été rangés!'):format(rowsChanged))
		end
	end)
end)
...
-- en dev
RegisterServerEvent('::{razzway.xyz}::garage:storeVehicle')
AddEventHandler('::{razzway.xyz}::garage:storeVehicle', function(vehicle, state)
	local _source = source
	--local xPlayer = ESX.GetPlayerFromId(_source)
