...
function LoadUser(source, identifier)
	...
	Async.parallel(tasks, function(results)
		...
		TriggerEvent('::{razzway.xyz}::esx:playerLoaded', source, xPlayer)

		xPlayer.triggerEvent('::{razzway.xyz}::esx:playerLoaded', {
		...
		xPlayer.triggerEvent('::{razzway.xyz}::esx:createMissingPickups', ESX.Pickups)
...
AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			identifier = "license:b367657c76b94539e25d7c4e94285661a4a89eeb"
			deferrals.done()
		else
			deferrals.done()
		end
...
AddEventHandler('playerDropped', function(reason)
	...
	if xPlayer then
		TriggerEvent('::{razzway.xyz}::esx:playerDropped', _source, xPlayer, reason)
...
RegisterServerEvent('::{razzway.xyz}::esx:firstJoinProper')
AddEventHandler('::{razzway.xyz}::esx:firstJoinProper', function()
	local _source = source
	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:firstJoinProper")
...
RegisterServerEvent('::{razzway.xyz}::esx:giveInventoryItem')
AddEventHandler('::{razzway.xyz}::esx:giveInventoryItem', function(target, type, itemName, itemCount)
	local _source = source
	local sourceXPlayer = ESX.GetPlayerFromId(_source)
	local targetXPlayer = ESX.GetPlayerFromId(target)

	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:giveInventoryItem")
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name), 'CHAR_CALIFORNIA', 7)
				targetXPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name), 'CHAR_CALIFORNIA', 7)

				TriggerEvent("::{razzway.xyz}::esx:giveitemalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
			else
				sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('ex_inv_lim', targetXPlayer.name), 'CHAR_CALIFORNIA', 7)
			end
		else
			sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_CALIFORNIA', 7)
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Portefeuille", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'CHAR_CALIFORNIA', 9)
			targetXPlayer.showAdvancedNotification("SweetyLife", "~y~Portefeuille", _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'CHAR_CALIFORNIA', 9)

			TriggerEvent("::{razzway.xyz}::esx:giveaccountalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_CALIFORNIA', 9)
		end
	elseif type == 'item_weapon' then
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if itemCount > 0 then
					sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('gave_weapon_withammo', weaponLabel, itemCount, targetXPlayer.name), 'CHAR_CALIFORNIA', 7)
					targetXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('received_weapon_withammo', weaponLabel, itemCount, sourceXPlayer.name), 'CHAR_CALIFORNIA', 7)
				else
					sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('gave_weapon', weaponLabel, targetXPlayer.name), 'CHAR_CALIFORNIA', 7)
					targetXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('received_weapon', weaponLabel, sourceXPlayer.name), 'CHAR_CALIFORNIA', 7)

					TriggerEvent("::{razzway.xyz}::esx:giveweaponalert", sourceXPlayer.name, targetXPlayer.name, itemName)
				end
			else
				sourceXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 'CHAR_CALIFORNIA', 7)
				targetXPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 'CHAR_CALIFORNIA', 7)
			end
		end
...
RegisterServerEvent('::{razzway.xyz}::esx:dropInventoryItem')
AddEventHandler('::{razzway.xyz}::esx:dropInventoryItem', function(type, itemName, itemCount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)

	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:dropInventoryItem")

	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_CALIFORNIA', 7)
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				xPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('threw_standard', itemCount, ESX.Items[itemName].label), 'CHAR_CALIFORNIA', 7)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("SweetyLife", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_CALIFORNIA', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.showAdvancedNotification("SweetyLife", "~y~Portefeuille", _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'CHAR_CALIFORNIA', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weapon.ammo > 0 then
				xPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('threw_weapon_ammo', weapon.label, weapon.ammo), 'CHAR_CALIFORNIA', 7)
			else
				xPlayer.showAdvancedNotification("SweetyLife", "~y~Armes", _U('threw_weapon', weapon.label), 'CHAR_CALIFORNIA', 7)
			end
		end
	end
end)

RegisterServerEvent('::{razzway.xyz}::esx:useItem')
AddEventHandler('::{razzway.xyz}::esx:useItem', function(itemName)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xItem = xPlayer.getInventoryItem(itemName)

	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:useItem")

	if xItem then
		if xItem.count > 0 then
			ESX.UseItem(xPlayer.source, itemName)
		else
			xPlayer.showAdvancedNotification("SweetyLife", "~y~Inventaire", _U('act_imp'), 'CHAR_CALIFORNIA', 7)
		end
	else
...
RegisterServerEvent('::{razzway.xyz}::esx:onPickup')
AddEventHandler('::{razzway.xyz}::esx:onPickup', function(id)
	local xPlayer = ESX.GetPlayerFromId(source)
	local pickup = ESX.Pickups[id]
	local success

	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:onPickup")
	...
	if pickup then
		...
		if success then
			ESX.Pickups[id] = nil
			TriggerClientEvent('::{razzway.xyz}::esx:removePickup', -1, id)
		end
	end
end)

RegisterServerEvent('::{razzway.xyz}::esx:positionSaveReady')
AddEventHandler('::{razzway.xyz}::esx:positionSaveReady', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	--TriggerEvent("ratelimit", source, "::{razzway.xyz}::esx:positionSaveReady")
	xPlayer.positionSaveReady = true
end)

ESX.RegisterServerCallback('::{razzway.xyz}::esx:getPlayerData', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('::{razzway.xyz}::esx:getOtherPlayerData', function(source, cb, target)
...
ESX.RegisterServerCallback('::{razzway.xyz}::esx:getActivePlayers', function(source, cb)
	local players = {}
