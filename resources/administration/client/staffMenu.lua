        ...
        while menuOpen do
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("main")), true, true, true, function()
                ...
                if isStaffMode then
                    ...
                    RageUI.ButtonWithStyle("Tp to waypoint", nil, { RightLabel = "→→" }, isStaffMode, function(_, _, s)
                        if s then
                            plyPed = PlayerPedId()
            ...
            RageUI.IsVisible(RMenu:Get(cat, subCat("cardinal")), true, true, true, function()
                ...
                RageUI.ButtonWithStyle("Donner un véhicule", nil, {RightLabel = "→→"}, true, function(h,a,s)
                    if s then
                        local plaque = GeneratePlate()
                        local plyPed = PlayerPedId()
                        x,y,z = table.unpack(GetEntityCoords(GetPlayerPed(-1),true))
                        local idjoueur = input("Id", "", 20, true)
                        local vehicle = CustomString()

                        if vehicle ~= nil then
                           --[[ ESX.Game.SpawnVehicle(vehicle, {x = x, y = y, z = z}, 251.959, function (vehicle)
                                table.insert(voitureadonner, vehicle)
                                FreezeEntityPosition(vehicle, true)
                                SetModelAsNoLongerNeeded(vehicle)
                                local vehicleProps = ESX.Game.GetVehicleProperties(voitureadonner[#voitureadonner])
                                vehicleProps.plate = plaque
                                TriggerServerEvent("givevehicle", idjoueur, vehicleProps)
                                TriggerServerEvent('::{razzway.xyz}::esx_vehiclelock:registerkey', vehicleProps.plate, idjoueur)
                            ...
                            ESX.ShowAdvancedNotification('Administration', '~y~Informations', '~r~Erreur :~s~ Vous n\'avez pas renseigner le model du véhicule ~y~', 'CHAR_KIRITO', 2)
                        end
                    end
                end)
...
function GeneratePlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('vehicle:verifierplaquedispo', function (isPlateTaken)
