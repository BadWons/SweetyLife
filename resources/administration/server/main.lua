...
ESX.RegisterServerCallback('vehicle:verifierplaquedispo', function (source, cb, plate)
...
RegisterServerEvent("givevehicle")
AddEventHandler("givevehicle", function(target, vehicleProps)
    local source = source
    local xPlayer = ESX.GetPlayerFromId(source)
    local targetp = ESX.GetPlayerFromId(target)
    if xPlayer.getGroup() == "superadmin" or xPlayer.getGroup() == "_dev" then
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", source, "~r~[GIVE] ~s~Vous avez donner un véhicule.")
        TriggerClientEvent("::{razzway.xyz}::esx:showNotification", target, "~r~[GIVE] ~s~Un véhicule vous a été attribué.")
        MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, state) VALUES (@owner, @plate, @vehicle, @state)', {
            ['@owner']   = targetp.identifier,
